  ...
  config = lib.mkMerge [
    # Common configuration
    {
      ...
      boot = {
        ...
        '';
        kernelModules = ["v4l2loopback" "cdc_acm"];
        blacklistedKernelModules = ["cytherm"];
        kernel.sysctl = {
      ...
      users.users.${username} = {
        isNormalUser = true;
        extraGroups = ["wheel" "docker" "wireshark" "input" "network" "dialout" "networkmanager" "ydotool"];
        shell = pkgs.zsh;
      };
    ...
    (lib.mkIf (machine == "desktop") {
      ...
      environment.systemPackages = basePackages ++ desktopPackages;
    })
    ...
    (lib.mkIf (machine == "laptop") {
      ...
      environment.systemPackages = basePackages ++ laptopPackages;
    })
  ];
}
